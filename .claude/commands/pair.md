...
argument-hint: "<issue> [--ad] [--mr N] [--cp] [--allow-unsafe-agents]"
allowed-tools: >-
...
1. Read the issue number from `$ARGUMENTS`. Without one, ask for it and stop.
...
3. If `--allow-unsafe-agents` is present, explain that an agent command removes
   ...
   externally isolated, credential-free environment; otherwise stop.
...
- *Questions.* Read `## Questions`. Put the first unanswered question to the
  ...
  on the user's behalf, or bundle multiple questions into one message.
...
the fix. The Git guard tripping is worth flagging loudly: it means an agent
tried to write history, and the repository should be inspected before
continuing.
